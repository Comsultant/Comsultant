  ...
  useEffect(() => {
    ...
    const fetchPriceData = async () => {
      ...
      if (payload?.data?.message === "success") {
        ...
        priceArr?.map(curr => {
          const item = {
            x: (curr[0] + "").replace(/(\d{4})(\d{2})(\d{2})/g, "$2.$3"),
            y: curr[1],
          };
          newArr.push(item);
        });
          ...
          <div className={style["product-info"]}>
            <div className={style["price-graph"]}>
              {priceData != undefined && priceData.length > 0 ? (
                <div className={style["month-button-box"]}>
                  <span
                    className={style["month-button"]}
                    onClick={() => setPeriod(3)}
                  >
                    3개월
                  </span>
              ...
              {isLogin ? (
                <>
                  <div className={style["right-box-mid"]}>
                    <span
                      className={style["shopping-button"]}
                      onClick={() =>
                        window.open(
                          `https://search.shopping.naver.com/search/all?query=${data?.name}`
                        )
                      }
                    >
                      네이버 쇼핑검색
                    </span>
                    {!data?.wish && !isWishClicked ? (
                      <HeartOutlined
                        style={{
                          fontSize: "40px",
                          cursor: "pointer",
                  ...
                  <div className={style["builder-select-box"]}>
                    ...
                    <div>
                      <button onClick={onBuilderAddClicked} className={style['add-button']}>추가</button>
                    </div>
                  </div>
                </>
              ) : null}
